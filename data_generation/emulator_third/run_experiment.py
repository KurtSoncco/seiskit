"""
Emulator_second: 8,100 simulations for surrogate training.

Fixed:
- Height = 100 m, Vs2 = 1500 m/s, no interlayer variability (flat surface).
- Lx = 1500 m with BC_width = 500 m, dx = 1 m, dz = 1 m, Lz = 60 m (50 soil + 10 bedrock).
...
def _load_recorder_txt(
    ...
    # Numeric Y order (shallow → deep is ascending y in ``center_node_y_positions``).
    # Lexicographic sort wrongly ordered e.g. ``row_y110.00`` before ``row_y2.00``.
