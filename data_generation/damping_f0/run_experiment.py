...
- Damping frequencies = (f0, 10.0) Hz
...
def run_case(index: int = 0):
    ...
    duration = 30.0  # 30 seconds (as per definition)
    ...
    # Compute f0
    f0 = Vs1 / (4 * thickness)
    damping_freqs = (f0, 10.0)

    # Create output directory
    output_dir = f"results/Vs1_{Vs1:.0f}_thickness_{thickness:.0f}_rH_{rH:.0f}_CV_{CV:.3f}_s{seed}_damping_method_{damping_method_idx}"
    ...
    task_id = f"{case_type}_Vs1{Vs1:.0f}_th{thickness:.0f}_rH{rH:.0f}_CV{CV:.3f}_s{seed}_damping_method_{damping_method_idx}"
    ...
    config = AnalysisConfig(
        ...
        motion_t_shift=0.5,
        damping_freqs=damping_freqs,  # (f0, 10.0) Hz
