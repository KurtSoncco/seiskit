...
- 3 different values of Vs1: 100, 280, 360 m/s
- 3 different values of thickness: 14, 50, 100 m
...
Total number of combinations: 3 * 3 * 2 * 5 * 2 = 180
"""
...
def run_case(index: int = 0):
    ...
    Vs1_list = [100, 280, 360]
    thickness_list = [14, 50, 100]  # Everything is a multiple of 2 meters
    CV_list = [0.1, 0.3]
    ...
    # Index structure: index = Vs1_idx * (3*3*2*5*2) + thickness_idx * (3*2*5*2) + CV_idx * (2*5*2) + seed_idx * (5*2) + damping_method_idx
    Vs1_idx = index // (
    ...
    output_dir = f"results/Vs1_{Vs1:.0f}_thickness_{thickness:.0f}_CV_{CV:.3f}_s{seed}_damping_method_{damping_method}"
    ...
    task_id = f"{case_type}_Vs1{Vs1:.0f}_th{thickness:.0f}_CV{CV:.3f}_s{seed}_damping_method_{damping_method}"
    ...
    plot_realization(
        ...
        title=f"Vs Realization (Vs1={Vs1:.0f} m/s, thickness={thickness:.0f} m, CV={CV:.3f}, seed={seed}, realization={realization_str}, damping_method={damping_method})",
    ...
    plot_damping_realization(
        ...
        title=f"Damping Realization (damping_method={damping_method}, CV={CV:.3f}, seed={seed}, realization={realization_str})",
        bedrock_mask=bedrock_mask_extended,
        config=config,
    )
