...
def test_analysis_task_creation():
    ...
    task = AnalysisTask(
        task_id="test_task", config=config, model_data=model_data, output_dir="/tmp/test"
    )
...
def test_analysis_result_success_property():
    ...
    # Successful result
    result1 = AnalysisResult(task_id="test_task", status="Finished successfully")
    assert result1.success is True

    # Failed result with error
    result2 = AnalysisResult(task_id="test_task", status="Failed", error="Some error")
    assert result2.success is False

    # Failed result without error
    result3 = AnalysisResult(task_id="test_task", status="Failed")
...
def test_run_parameter_study():
    ...
    _parameter_variations = {"hx": [5.0, 2.5], "motion_freq": [0.5, 0.75]}
    ...
    tasks = prepare_analysis_tasks(
        [{"task_id": "test", **base_config, "hx": 5.0, "motion_freq": 0.5}], material_data
    )
...
def test_collect_results():
    ...
    results = [
        AnalysisResult(task_id="task1", status="Finished successfully", execution_time=10.0),
        AnalysisResult(task_id="task2", status="Failed", error="Some error", execution_time=5.0),
        AnalysisResult(task_id="task3", status="Finished successfully", execution_time=15.0),
    ]
