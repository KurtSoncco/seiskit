...
def test_compute_quality_factor():
    ...
    expected_Q_low = 7.17 + 0.00276 * Vs_low
    ...
    # Test boundary case
    Vs_boundary = 800.0
    Q_boundary = compute_quality_factor(Vs_boundary)
    assert Q_boundary == 50.0
    
    # Test very low Vs
    Vs_very_low = 50.0
    Q_very_low = compute_quality_factor(Vs_very_low)
    expected_Q_very_low = 7.17 + 0.00276 * Vs_very_low
...
def test_compute_quality_factor_range():
    ...
    for Vs in [100.0, 200.0, 400.0, 600.0, 799.0]:
        ...
        expected_Q = 7.17 + 0.00276 * Vs
        assert abs(Q - expected_Q) < 1e-10
        assert 7.17 < Q < 50.0
    
    # Test multiple values at or above 800
    for Vs in [800.0, 900.0, 1000.0, 1500.0]:
        Q = compute_quality_factor(Vs)
        assert Q == 50.0
...
def test_absorbing_boundary_elements_no_damping():
    ...
    from seiskit.builder import ModelData, SoilElementData, BoundaryElementData
...
def test_rayleigh_damping_only_applied_to_soil_elements():
    ...
    for tag in boundary_tags:
        model_data.boundary_elements.append(
            BoundaryElementData(tag=tag, nodes=(tag, tag+1, tag+2, tag+3), btype="L", G=1e6, poiss=0.3, rho=2000.0)
        )
