...
def test_complete_workflow():
    ...
    config = AnalysisConfig(Ly=140.0, Lx=260.0, hx=5.0, duration=15.0, motion_freq=0.75)
    ...
    ricker_wave = compute_ricker(
        freq=config.motion_freq, t_shift=1.4, duration=config.duration, dt=0.01
    )
    ...
    alphaM, betaK = compute_rayleigh_coefficients(
        zeta=config.damping_zeta, f1=config.damping_freqs[0], f2=config.damping_freqs[1]
    )
...
def test_material_properties_loading():
    ...
    material_data = {"vs": vs_data, "rho": rho_data, "nu": nu_data}
...
def test_configuration_validation():
    ...
    custom_config = AnalysisConfig(Ly=200.0, Lx=300.0, hx=2.5, duration=20.0, motion_freq=1.0)
...
def test_parallel_processing_interface():
    ...
    from seiskit.parallel import AnalysisResult

    # Test AnalysisResult creation
    result = AnalysisResult(
        task_id="test_task", status="Finished successfully", execution_time=10.5
    )
    ...
    error_result = AnalysisResult(
        task_id="failed_task", status="Failed", error="Some error occurred"
    )
