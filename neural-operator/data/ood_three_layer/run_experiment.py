"""Runner for Tier B ood_three_layer OpenSees data generation.

Index → flat 3-layer GRF → OpenSees 2D → run_{index}.h5 (training-data style).
"""
...
def _write_h5(
    ...
    with h5py.File(h5_path, "w") as handle:
        ...
        params.attrs["H1_requested"] = entry.H1_requested
        ...
        params.attrs["H2_requested"] = entry.H2_requested
...
def run_case(entry: OodThreeLayerEntry, manifest_path: Path) -> str:
    t0 = time.time()
    task_id = f"{CASE_TYPE}_run{entry.index}"
    ...
    print(
        f"  Vs1={entry.Vs1:.1f} H1={entry.H1_discretized:.1f} CoV={entry.CoV1:.3f} "
        f"rH={entry.rH1:.1f} aHV={entry.aHV1:.1f}"
    )
    print(
        f"  Vs_mid={entry.Vs_mid:.1f} H2={entry.H2_discretized:.1f} "
        f"Vs_bedrock={entry.Vs_bedrock:.1f} contrast={entry.Vs_contrast:.3f} "
        f"Lz={entry.Lz_discretized:.1f}"
    )

    Vs_extended, _x, _z, _h, bedrock_mask = create_three_layer_vs_realization(
        Vs1=entry.Vs1,
        Vs_mid=entry.Vs_mid,
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Run Tier B ood_three_layer generation.")
