#!/bin/bash
# Run one ood_dipping index on Stampede3.
#   INDEX=0 sbatch neural-operator/data/ood_dipping/stampede3_single_index.sh
#SBATCH -J ood_dip_idx
#SBATCH -o stampede3_ood_dipping_idx.o%j
#SBATCH -e stampede3_ood_dipping_idx.e%j
#SBATCH -p skx
#SBATCH -N 1
#SBATCH -n 1
...
RUN_BASE="${RUN_BASE:-${SCRATCH:-${PROJECT_ROOT}}/opensees_ood_dipping_test}"

export OOD_OUTDIR="${OOD_OUTDIR:-${RUN_BASE}/raw_runs}"
export OOD_H5_DIR="${OOD_H5_DIR:-${RUN_BASE}/h5}"
export OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1
export PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="${PROJECT_ROOT}:${PYTHONPATH:-}"

mkdir -p "${RUN_BASE}" "${OOD_OUTDIR}" "${OOD_H5_DIR}"
...
CMD=("${PYTHON_BIN}" -u "${RUNNER_PY}" --manifest-path "${MANIFEST_PATH}" --index "${INDEX}")
if [ "${FORCE_RERUN}" = "1" ]; then CMD+=(--force); fi
"${CMD[@]}"
