...
if [ -n "${SLURM_TMPDIR:-}" ]; then
  ...
  RESULTS_BASE="/tmp/rv_${USER}_job_${SLURM_JOB_ID}_task_${TASK_ID}"
else
...
# Optional: stage static inputs to node scratch (not required for this campaign).
if [ -n "${SLURM_TMPDIR:-}" ]; then
...
export RV_OUTDIR=/global/scratch/users/$USER/rv_comparison/opensees_runs/${SLURM_JOB_ID:-0}_${TASK_ID}
...
mkdir -p "$PARALLEL_HOME" "$TMPDIR" "$RV_OUTDIR" "$RV_H5_DIR"
...
# Per-sim timeout (10h) so one stuck 2D run doesn't hang the slot; job time is 10h.
SIM_TIMEOUT=36000
export SIM_TIMEOUT
